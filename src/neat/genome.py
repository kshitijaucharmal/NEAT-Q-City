...
class Genome:
    ...
    def backpropogate(self, inputs, targets):
        ...
        for k in range(len(inputs)):  # loop for batched inputs
            ...
            self.d_loss_output = [
                (targets[k][i] - self.predicted_output[k][i])
                for i in range(len(targets[0]))
            ]
            self.d_output_input3 = [
                self.d_loss_output[i]
                * self.sigmoid_derivative(self.Output_Layer[i].sum)
                for i in range(len(targets[0]))
            ]
            for i in range(len(self.Output_Layer)):
                for j in range(len(self.Output_Layer[i].in_genes)):
                    ...
                    if self.temp_gene.enabled == True:
                        self.temp_gene.error = (
                            self.d_output_input3[i] * self.temp_gene.in_node.output
                        )
                        # updating weights
                        self.temp_gene.weight += self.lr * self.temp_gene.error
                        # print(f"updating weights..")
                        # storing the required value in node
                        self.temp_gene.in_node.value += (
                            self.d_output_input3[i] * self.temp_gene.weight
                        )
                        # print(f"updating value in nodes..")

            # Compute gradients of loss with respect to second hidden layer
            self.d_output_input2 = [
                self.HL_2[i].value * self.sigmoid_derivative(self.HL_2[i].sum)
            ...
            for i in range(len(self.HL_2)):
                for j in range(len(self.HL_2[i].in_genes)):
                    ...
                    if self.temp_gene.enabled == True:
                        self.temp_gene.error = (
                            self.d_output_input2[i] * self.temp_gene.in_node.output
                        )
                        # updating weights
                        self.temp_gene.weight += self.lr * self.temp_gene.error
                        # print(f"updating weights..")
                        # storing the required value in node
                        self.temp_gene.in_node.value += (
                            self.d_output_input2[i] * self.temp_gene.weight
                        )
                        # print(f"updating value in nodes..")

            # Compute gradients of loss with respect to first hidden layer
            self.d_output_input1 = [
                self.HL_1[i].value * self.sigmoid_derivative(self.HL_1[i].sum)
            ...
            for i in range(len(self.HL_1)):
                for j in range(len(self.HL_1[i].in_genes)):
                    ...
                    if self.temp_gene.enabled == True:
                        self.temp_gene.error = (
                            self.d_output_input1[i] * self.temp_gene.in_node.output
                        )
                        # updating weights
                        self.temp_gene.weight += self.lr * self.temp_gene.error
                        # print(f"updating weights..")
                        # storing the required value in node
                        self.temp_gene.in_node.value += (
                            self.d_output_input1[i] * self.temp_gene.weight
                        )
                        # print(f"updating value in nodes..")
